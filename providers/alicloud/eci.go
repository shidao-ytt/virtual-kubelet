...
func (p *ECIProvider) CreatePod(ctx context.Context, pod *v1.Pod) error {
	...
	log.Printf("CreateContainerGroup successed. %s, %s, %s\n", response.RequestId, response.ContainerGroupId, ContainerGroupName)
	return nil
}
...
func (p *ECIProvider) DeletePod(ctx context.Context, pod *v1.Pod) error {
	...
	if eciId == "" {
		return errors.New(fmt.Sprintf("DeletePod cann't find Pod %s-%s", pod.Namespace, pod.Name))
	}
...
func (p *ECIProvider) GetContainerLogs(ctx context.Context, namespace, podName, containerName string, tail int) (string, error) {
	...
	for i := 0; i < retry; i++ {
		...
		if err != nil {
			log.Printf("Error getting container logs, retrying\n")
...
func (p *ECIProvider) GetPods(ctx context.Context) ([]*v1.Pod, error) {
	...
	for _, cg := range p.GetCgs() {
		...
		if err != nil {
			log.Println("error converting container group to pod ", cg.ContainerGroupId, err)
			continue
		}
